...
const CardValidityCheckout = () => {
  ...
  // Always holds the latest billing details for the wallets handler closure
  ...
  // ── Card validity gate ───────────────────────────────────────────────────
  // registerCardDetailsHandler is NOT on the prototype — the SDK attaches it
  // to the instance about a second after the custom element is defined. So
  // neither querySelector nor customElements.whenDefined() is sufficient;
  // we poll until the method actually exists.
  useEffect(() => {
    if (!sessionToken) return;
    let registered = false;

    const interval = setInterval(() => {
      const checkoutElement = document.querySelector('super-checkout');
      if (checkoutElement && typeof checkoutElement.registerCardDetailsHandler === 'function' && !registered) {
        registered = true;
        clearInterval(interval);

        checkoutElement.registerCardDetailsHandler((event) => {
          const { cardDetailsValid } = event.detail;
          console.log('[CardValidity] cardDetailsValid:', cardDetailsValid);
          setIsCardValid(!!cardDetailsValid);
        });
        console.log('✅ [CardValidity] registerCardDetailsHandler registered');
      }
    }, 100);
  ...
  return (
    <div className="layout-two-col">
      ...
      <div style={{ backgroundColor: '#fff', padding: '30px', borderRadius: '16px', border: '1px solid #ddd' }}>
        <h2 style={{ fontSize: '1.4rem', marginBottom: '25px' }}>Embedded Checkout — Card Validity</h2>
        {sessionToken ? (
          <>
            <super-checkout
              key={refreshKey}
...
export default CardValidityCheckout;